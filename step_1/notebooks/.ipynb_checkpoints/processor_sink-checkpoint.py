...
CSV_BASE_PATH = "/home/jovyan/processed_data"  # Base path for CSV files inside the Jupyter container

# --- Note on CSV Path ---
# '/home/jovyan/' is the home directory within the jupyter/scipy-notebook container.
# The './notebooks' volume maps your local 'notebooks' folder to '/home/jovyan/'.
# So, the CSV files will appear in the 'notebooks' folder on your host machine.
...
def write_to_csv(data, filepath):
    ...
    try:
        with open(filepath, "a", newline="", encoding="utf-8") as csvfile:
            # Dynamically determine headers based on the keys in the flattened data
            # Note: This assumes keys are consistent enough for a CSV structure.
            # For highly variable structures, another format (like JSON lines) might be better.
...
def main():
    ...
    logger.info(
        f"Input topics: {INPUT_TOPICS}, Output topic: {OUTPUT_TOPIC}, Sink base path: {CSV_BASE_PATH}"
    )
            ...
            if sink_messages:
                ...
                for topic_partition, messages in sink_messages.items():
                    for message in messages:
                        ...
                        data_type = data.get(
                            "type", "unknown"
                        )  # Get the type, default to 'unknown'
                        specific_csv_path = f"{CSV_BASE_PATH}_{data_type}.csv"
                        write_to_csv(data, specific_csv_path)
                        written_count += 1

                # Commit offsets for the sink consumer after writing a batch
                sink_consumer.commit()
                logger.info(
                    f"Wrote {written_count} processed records to respective CSV files."
                )
